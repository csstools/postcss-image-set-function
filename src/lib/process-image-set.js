...
export default (imageSetOptionNodes, decl) => {
	...
	while (index < length) {
		...
		if (!comma) {
			return handleInvalidation('unexpected comma', imageSetOptionNodes[index])
		} else if (!value) {
			return handleInvalidation('unexpected image', imageSetOptionNodes[index + 1])
		} else if (!media) {
			return handleInvalidation('unexpected resolution', imageSetOptionNodes[index + 2])
		}
